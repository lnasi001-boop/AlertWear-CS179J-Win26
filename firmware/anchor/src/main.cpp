/*
 * VERTEX UWB - Anchor 0 (A0) Simple Test
 * 
 * Based on MakerFabs esp32s3_at_a0 example
 * Configures board as Anchor 0 and outputs distance data to Serial
 * 
...
// ============== USER CONFIG ==============
#define UWB_INDEX 0          // Anchor ID (0-7)
#define UWB_TAG_COUNT 64     // Max tags to track
// =========================================
...
void setup() {
    ...
    SERIAL_LOG.println(F("  VERTEX UWB - Anchor 0 Test"));
    SERIAL_LOG.println(F("========================================\n"));
    ...
    SERIAL_LOG.println(F("Configuring UWB module as Anchor 0..."));
    ...
    SERIAL_LOG.println(F("\n========================================"));
    SERIAL_LOG.println(F("  Anchor 0 Ready - Waiting for Tags"));
    SERIAL_LOG.println(F("========================================\n"));
    
    display.clearDisplay();
    display.setTextSize(1);
    display.setCursor(0, 0);
    display.println("Anchor 0 READY");
    display.println("Waiting for tags...");
    display.display();
}
        ...
        if (c == '\r') {
            ...
            if (response.length() > 0) {
                ...
                if (response.startsWith("AT+RANGE") || response.indexOf(":") > 0) {
                    ...
                    display.setTextSize(1);
                    display.setCursor(0, 0);
                    display.println("Anchor 0 - RANGING");
                    display.println();
                    display.setTextSize(1);
